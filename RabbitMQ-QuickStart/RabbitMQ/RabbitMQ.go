...
func NewRabbitSimple(queueName string) *RabbitMQ {
	rbmq := NewRabbitMQ(queueName, "", "")
	var err error

	rbmq.conn, err = amqp.Dial(rbmq.Mqurl)
	rbmq.failOnErr(err, "fail to connect rabbit itmq!")

	rbmq.channel, err = rbmq.conn.Channel()
	rbmq.failOnErr(err, "fail to open a channel")
...
// ReceiveSub subscriber start to receive message
...
func NewRabbitMQRouting(exchange, routingKey string) *RabbitMQ {
	rbq := NewRabbitMQ("", exchange, routingKey)
	var err error

	rbq.conn, err = amqp.Dial(rbq.Mqurl)
	rbq.failOnErr(err, "failed to connect rabbitmq!")
	//获取channel
	rbq.channel, err = rbq.conn.Channel()
	rbq.failOnErr(err, "failed to open a channel")
	return rbq
}
